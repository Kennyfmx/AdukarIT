...
console.log(citiesFilter("California"));

//Подсчитайте, сколько миллионов населения живёт во всех городах на широте от 25 до 30 градусов.

function populationCensus(a, b) {
	let census = 0;
	let cities = data.filter(function(el) {
		if (el.latitude >= a && el.latitude < b) {
			census += parseFloat(el.population)
		}
	});
	return "Количество людей, проживающих на данной территории " + census;
}
console.log(populationCensus(25, 30));
...
function sort(byLetter) {
	let sortedCity = data.filter(function(el) {
		if (el.city[0] == byLetter) {
			return true;
			}
		});
	return sortedCity.sort(function(a, b) {
		if (a.city > b.city) {
			return 1
		}
...
function arrayConversion() {
	...
	for (let key in states) {
		for (let a = 0; a < data.length; a++) {
			if (key == data[a].state) {
				states[data[a].state].push({ 
					city: data[a].city,
					population: data[a].population,
					rank: data[a].rank
				});
			}
		}
